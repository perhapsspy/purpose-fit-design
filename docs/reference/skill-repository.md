...
- 이 저장소는 `purpose-fit-design`라는 독립 스킬 패키지다.
- 기본 README 언어는 한국어이고, 영어 문서는 `README.en.md`에 둔다.
- 설치 가능한 스킬은 `skills/purpose-fit-design/` 아래에 둔다.
- `SKILL.md`는 영문 base skill이며 frontmatter metadata를 가진다.
- `SKILL.ko.md`는 한국어 페어이며 frontmatter를 넣지 않는다.
- `skills/purpose-fit-design/agents/openai.yaml`은 OpenAI-facing interface metadata를 제공한다.
- 프로젝트 라이선스는 MIT다.
...
- 스킬 본문은 목적 적합성 판단을 맡고, 모든 owner discovery, semantic boundary design, implementation planning을 직접 대체하지 않는다.
