...
description: Use to set an early design or implementation-planning direction from user/domain purpose, explicit constraints, current evidence, and verifiable success conditions. Apply to new features, reuse decisions, and temporary implementations when existing code, precedent, defaults, or urgency could steer the choice. Skip mechanical edits, read-only source discovery, clear specialist cases, and coding after direction is settled.
---
...
End with a provisional direction, the smallest useful check, or one question that blocks the choice. Finding more possibilities does not expand implementation scope.
...
Investigate or ask one focused question when a missing fact changes the choice. Do not fill the gap with the current implementation.
...
Treat explicit user corrections as binding. Apply them at the reach implied by the request, preserve unaffected constraints, and do not reintroduce a rejected concept under another name.
...
Skip this skill when a specialist problem is already clear. Otherwise, hand off only when an unresolved fact blocks the direction:
